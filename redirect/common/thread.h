...
public:
	...
	bool create(unsigned int threadnum,void(*entry)(Thread* ,void*),void* user){
		for(unsigned int n=0;n<threadnum;n++){
			Thread * t = new Thread(entry,user);
			t->start();
	...
	void destroy(){
		for(unsigned int n=0;n<_threadlist.size();n++){
			Thread * t = _threadlist[n];
			t->stopAndWait();
			delete t;
		}
		_threadlist.clear();
