...
gtr::GameTaskletT::operator==(const GameTaskletT& __rhs) const
{
    ...
    if(proxyEnable != __rhs.proxyEnable)
    {
        return false;
    }
    if(proxyRegion != __rhs.proxyRegion)
    {
        return false;
    }
    if(remainTime != __rhs.remainTime)
    {
        return false;
    }
    if(post != __rhs.post)
...
gtr::GameTaskletT::operator<(const GameTaskletT& __rhs) const
{
    ...
    else if(__rhs.proxyEnable < proxyEnable)
    {
        return false;
    }
    if(proxyRegion < __rhs.proxyRegion)
    ...
    else if(__rhs.proxyRegion < proxyRegion)
    {
        return false;
    }
    if(remainTime < __rhs.remainTime)
    ...
    else if(__rhs.remainTime < remainTime)
    {
        return false;
    }
    if(post < __rhs.post)
...
gtr::GameTaskletT::__write(::IceInternal::BasicStream* __os) const
{
    ...
    __os->write(proxyEnable);
    __os->write(proxyRegion);
    __os->write(remainTime);
    post.__write(__os);
    audit.__write(__os);
    verify.__write(__os);
}
...
gtr::GameTaskletT::__read(::IceInternal::BasicStream* __is)
{
    ...
    __is->read(secretKey);
    __is->read(proxyEnable);
    __is->read(proxyRegion);
    __is->read(remainTime);
    post.__read(__is);
    audit.__read(__is);
    verify.__read(__is);
}
...
static const ::std::string __gtr__IQuestionServer_ids[3] =
{
    "::Ice::Object",
    "::gtr::IQuestionServer",
    "::gtr::IService"
...
{
    return ::std::binary_search(__gtr__IQuestionServer_ids, __gtr__IQuestionServer_ids + 3, _s);
}

::std::vector< ::std::string>
gtr::IQuestionServer::ice_ids(const ::Ice::Current&) const
{
    return ::std::vector< ::std::string>(&__gtr__IQuestionServer_ids[0], &__gtr__IQuestionServer_ids[3]);
}
...
{
    ::std::pair< ::std::string*, ::std::string*> r = ::std::equal_range(__gtr__IQuestionServer_all, __gtr__IQuestionServer_all + 14, current.operation);
    if(r.first == r.second)
    {
        throw ::Ice::OperationNotExistException(__FILE__, __LINE__, current.id, current.facet, current.operation);
    }
