...
201.4.27 zhangbin
1.GameTaskResultT增加 textval用于支持帐号审核回带信息
2.GameInfoT 增加maskType 游戏业务能力掩码


2010.5.12 zhangbin
1. IQuestionServer增加对IService接口支持
*/
...
struct QuestionContentT{
	int type;								// 2- 密保卡(图片+文本坐标), 1- 密保卡(图片+图片坐标);3-验证码(图片)
...
interface IQuestionServer extends IService{
	/*************************************************
		answer()
		机器人提交答题请求
		@params:
...
enum GameWorkSheetTypeT{
	...
	gwsVerify				//验证
			...
			*/
const int gwsPost = 0x01;
const int gwsAccountAudit =0x02;
const int gwsVerify = 0x04;
...
struct GameTaskletT{
	...
	bool	proxyEnable;	//代理是否启用
	string proxyRegion;	//代理区域
	
	int remainTime;	//做单剩余时间
...
struct GameInfoT{
	...
	string name;		//游戏名称 
	string version;	//游戏版本
	int 	maskType;	//业务类型掩码，例如: 邮寄，审核 ，验证 ，游戏机器人必须提供做单功能(不同功能 or 组合 )
};
