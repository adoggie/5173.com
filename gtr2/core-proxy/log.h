...
#ifndef _SW_DEBUGLOG_H
#define _SW_DEBUGLOG_H
...
public:
	...
	public:
		...
		int 	Write2(const TCHAR * wintitle,const char * data,unsigned int len){			
		...
		WindowWriter(const TCHAR * wintitle,bool refind=false){
			_win_title = wintitle;
			_hwnd = NULL;
			_type = WINDOW;
			_refind = refind;
	...
	public:
		...
		virtual int 	Write(const char * data,unsigned int len){
			...
			ret = (int)fwrite(data,(size_t)len,1,_fp);
			fflush(_fp);
			_cur_size += len;
			return len;
		}
	...
	#define LOG_MAX_BUFF_SIZE 1024*400
...
inline swLogger* swLogger::instance(){
	static swLogger	* _handle = NULL;
	if( _handle  == NULL){
		_handle = new swLogger;
	}
	return _handle;
}

inline 
void	swLogger::PrintString(int level,const char * data){
	char *levelname;	
	if( !(level & _level) ){	
		return;	
	}
...
/*
	...
	msg.append(" "+ std::string(levelname)+" ");
	if( occur = (char*)strstr( format.c_str(),(const char*)mark) ) {

		format.replace(occur-format.c_str(),len,asctime(localtime(0)));
	}
	mark = "<MESSAGE>";
	len =(int) strlen(mark);
...
*/
	...
	for( itr= _writer_list.begin();itr!=_writer_list.end();itr++){
		(*itr)->Write(msg.c_str(),(size_t)msg.size());
	}	
	//_writer_lock.Unlock();
}
...
// 
 #define LOG_ADD_UDP_WRITER(log,host,port)	\
 	swlib::swLogger::UdpWriter * w_udp = new swlib::swLogger::UdpWriter(host,port);\
 	w_udp->Open();\
 	(log).SetWriter(w_udp);
 
 #define LOG_ADD_WINDOW_WRITER(log,title)	\
 	swlib::swLogger::WindowWriter * w_win = new swlib::swLogger::WindowWriter(title);\
 	w_win->Open();\
 	(log).SetWriter(w_win);

#define LOG_ADD_REGFILE_WRITER(log,logfile)	\
	swlib::swLogger::RegFileWriter * w_file = new swlib::swLogger::RegFileWriter(logfile);\
	w_file->Open();\
	(log).SetWriter(w_file);




#define SWLIB_LOGGER_INIT()  
//	swlib::swLogger	* swlib::swLogger::_handle = NULL;
